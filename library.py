...
def get_user_checkouts(user_id):
    cur.execute("SELECT b.ISBN, b.Title, c.DayOut, c.DayDue, c.DayReturned FROM Checkout c JOIN Book b ON c.ISBN = b.ISBN WHERE c.Lib_ID = %s ORDER BY c.DayDue ", [user_id])
    rows = cur.fetchall()
    return rows
...
def book_lookup():
    ...
    with ui.row():
        ...
        def click_book(e):
            ...
            app.storage.user['selected_book'] = selected_book
